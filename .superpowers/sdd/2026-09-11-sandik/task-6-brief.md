# Görev 6 — brief (kaynak: docs/superpowers/plans/2026-09-11-sandik.md)
...
**Interfaces:**
- Consumes: Görev 4'teki `M` fonksiyonları; `Ortak.tarihYaz`, `Ortak.yuzdeYaz`, `Ortak.sayiYaz`. `M.meclisDurumu(k)` şunları döndürür: `{girenler, giremeyenler, bilinmeyen, bagimsiz, diger}`. Buradaki `bilinmeyen`, sandalyesi `null` olan partilerdir; panelde "Sandalye bilgisi yok" başlığıyla listelenir (kullanıcı kararı).
