...
- Soyağacında kart sayısı 66, bağ sayısı 73 — kullanıcı onaylı parti eklemeleri dışında değişmez.
  **Güncel (Görev 10, 2026-09-25):** kullanıcı onaylı altı eklemeyle kart 72, bağ 77 (`araclar/denetim/soyagaci.js` bu değerleri veriyor).
...
(function (kok, fabrika) {
  ...
  function tarihYaz(iso, kisa) {
    if (!iso) return "—";
    // günü bilinmeyen tarih "YYYY-AA" olabilir (kullanıcı kararı, 2026-09-11)
...
- Produces: `SandikMantik.olustur(P, O) → M` (tarayıcıda `window.SandikMantik`, Node'da `require("../../sandik-mantik")`). `M`'nin fonksiyonları:
  ...
  - `meclisDurumu(kayit) → {girenler: satir[], giremeyenler: satir[], bilinmeyen: satir[], bagimsiz: satir|null, diger: satir|null}` — `bilinmeyen`: sandalyesi `null` olan partiler (kullanıcı kararı, 2026-09-11; Görev 4 düzeltme turu 1'de eklendi, üç ek testle birlikte).
...
function dogrulaSandik(P, S) {
  ...
  S.secimler.forEach((k, i) => {
    ...
    // günü bilinmeyen tarih "YYYY-AA" olabilir; başka biçim kabul edilmez
...
```html
    <section id="sandik">
      ...
      <ul>
        ...
        <li>Askerî yönetim dönemlerinde (27 Mayıs 1960 ve 12 Eylül 1980 sonrasında, askerî yönetimden
          çıkışı sağlayan genel seçime kadar) yapılan referandumlar ve kurulan hükümetler, kırmızı
          kesinti bandının altında toplanır; geçiş seçimi sıradaki dönemi açar.</li>
      </ul>
    </section>
```
